...
import { ChevronRight, ChevronLeft, Sparkles, Trophy, Star, Zap, Heart, Award, Target, Flame, Home } from 'lucide-react'
...
export default function AssessmentQuestionnaire() {
  ...
  // Badge definitions - reduced to fewer badges
  ...
  useEffect(() => {
    // Show realm character when entering new realm - reduced time
    if (currentQuestionIndex === 0 && currentRealmIndex > 0) {
      setRealmCharacter(realmCharacters[currentRealmIndex])
      setTimeout(() => setRealmCharacter(null), 1500)
    }
  }, [currentRealmIndex])
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      >
        ...
        {/* Progress Bar with Realm Indicators */}
        <div className="container mx-auto mt-4">
          <div className="relative">
            {/* Realm markers */}
            <div className="flex justify-between mb-2">
              {realms.map((realm, index) => {
                const realmProgress = (index / realms.length) * 100
                const isCompleted = currentRealmIndex > index
                const isCurrent = currentRealmIndex === index
                
                return (
                  <motion.div
                    key={realm.id}
                    className="flex flex-col items-center"
                    style={{ width: `${100 / realms.length}%` }}
                  ...
                  >
                    <motion.div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 ${
                        isCompleted
                          ? 'bg-gradient-to-br from-cyan-400 to-purple-500 border-cyan-400'
                          : isCurrent
                          ? 'bg-gradient-to-br from-magenta-500 to-purple-600 border-magenta-400'
            ...
            <div className="flex justify-between mt-2 text-xs text-gray-400">
              <span className="text-magenta-400 font-semibold">{currentRealm.name}</span>
      ...
      {/* Main Content */}
      <div className="relative z-10 container mx-auto px-4 py-12 flex items-center justify-center min-h-[calc(100vh-200px)]">
